...
import { productosConOferta, sanitizarOfertasCliente } from '../utils/productos.js';
...
function idsOfertaActivaDelRestaurante(
  ...
  return new Set(
    ofertasZona
      .filter((o) => Number(o.idRestaurante) === idRestaurante)
      .map((o) => o.producto?.idProducto)
      .filter((id): id is number => id != null),
  );
}

export function useMenuRestaurante(idRestauranteParam: number | string | undefined) {
  ...
  const cargar = useCallback(async () => {
    ...
    try {
      ...
      setIdsOfertaActiva(idsOfertaActivaDelRestaurante(ofertasZona, idRestaurante));
      setMenu(data);
    } catch (e: unknown) {
