...
const cardBase =
  'block shrink-0 overflow-hidden rounded-[18px] bg-trego-card shadow-[0_2px_8px_rgba(0,0,0,0.08)] transition hover:shadow-[0_3px_12px_rgba(0,0,0,0.12)]'

function formatearPrecio(n) {
  return `$ ${Number(n).toLocaleString('es-UY', { maximumFractionDigits: 0 })}`
}

export default function OfertaPlatoCard({ oferta, enGrid = false }) {
  const producto = oferta.producto ?? {}
  const idRestaurante = oferta.idRestaurante ?? producto.idRestaurante
  const { conDescuento, original, tieneOferta } = obtenerPrecios(producto)
  const sizeClass = enGrid ? 'aspect-square w-full' : 'aspect-square w-[158px] sm:w-[172px]'
  const imagen =
    producto.fotoPlato ??
    producto.urlImagen ??
